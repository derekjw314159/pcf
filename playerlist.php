...
require_once $_SERVER['DOCUMENT_ROOT'] . '/pcf/class.county.php';
$user_home = new USER();
$county_home = new COUNTY();
...
/* Logic for default county
 */
if (isset($_REQUEST['county'])){
	$county = $_REQUEST['county'];
	}
else{
	$county = $userrow['userCounty'];
	}
// Check it exists
$data = $county_home->Read();
$countyrow = $data[0];
for($i=0; $i<count($data) ; $i++){
	if ($county == $data[$i]['userCounty']){
			...
			<div class="nav-collapse collapse" style="display: block">
				<ul class="nav pull-right">
					<li class="dropdown">
						...
						<?php echo $userrow['userEmail']; ?> <i class="caret"></i>
						</a>
